                  ...
                  child: Center(
                    child: Text(
                      ...
                      style: const TextStyle(
                          color: Colors.white, fontSize: 7, height: 1.1),
                      maxLines: 1,
      ...
      return Image.file(
        ...
        frameBuilder: (context, child, frame, wasSynchronouslyLoaded) {
          ...
          return const Icon(Icons.image,
              color: Colors.grey, size: 24); // ロード中プレースホルダー
        },
      );
    } else if (ext == 'svg') {
      ...
      return FutureBuilder<String>(
        ...
        builder: (context, snapshot) {
          if (snapshot.hasData &&
              ...
              (isTextExt || snapshot.data!.length > 10)) {
            return Padding(
              padding: const EdgeInsets.all(2.0),
              child: Text(
                snapshot.data!,
...
class _PdfPreview extends StatefulWidget {
  ...
  const _PdfPreview(
      {required this.file, required this.l10n, this.isThumbnail = false});
...
Future<List<String>> _listArchiveFiles(io.File file) async {
  try {
    ...
    return archive.files
        .take(20)
        .map((f) =>
            '  ${f.name}${(!f.isFile || f.name.endsWith('/')) ? "" : ""}')
        .toList();
  } catch (e) {
    return [];
  }
...
Future<String> _readText(io.File file, AppLocalizations l10n,
