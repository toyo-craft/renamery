...
class DirectoryProvider extends ChangeNotifier {
  ...
  Future<void> checkForUpdates() async {
    ...
    try {
      ...
      final request = await client.getUrl(Uri.parse(
          'https://api.github.com/repos/toyo-craft/renamery/releases/latest'));
  ...
  Future<void> init() async {
    ...
    _touchMode = s.getBool('touchMode') ??
    ...
    if (targetDir != null && await _directoryExistsWithTimeout(targetDir)) {
  ...
  void _saveState() {
    ...
    s.set('isLicenseAccepted', _isLicenseAccepted,
  ...
  void prevEnlargedPreview() {
    ...
    int selectedIdx = selectedFiles.indexOf(currentFile!);
    selectedIdx =
        (selectedIdx - 1 + selectedFiles.length) % selectedFiles.length;
    _enlargedPreviewIndex = _currentFiles.indexOf(selectedFiles[selectedIdx]);
    notifyListeners();
  }
  ...
  bool isEtcMode(RenameMode mode) =>
      [RenameMode.changeTimestamp, RenameMode.changeAttributes].contains(mode);
    ...
    if (reader.canProvide(Formats.fileUri)) {
      ...
      for (final item in reader.items) {
        if (item.canProvide(Formats.fileUri)) {
          ...
          try {
            final srcP = uri.toFilePath();
            final name = p.basename(srcP);
            final destP = _getUniquePath(p.join(_currentDirectory!.path, name));
            final ent =
                FileSystemEntity.typeSync(srcP) == FileSystemEntityType.file
                    ? File(srcP)
                    : Directory(srcP);
            if (_isCutMode) {
  ...
  Future<void> createNewFolder() async {
    if (_currentDirectory == null) return;
    String folderName = '新しいフォルダー';
    int index = 1;
    while (
        await Directory(p.join(_currentDirectory!.path, folderName)).exists()) {
      index++;
      folderName = '新しいフォルダー ($index)';
    }
    try {
      final destP = p.join(_currentDirectory!.path, folderName);
      await Directory(destP).create();
      _undoManager
          .addTransaction([UndoAction('', destP, type: UndoType.create)]);
      await refresh();
    } catch (e) {
  ...
  void updateFilterSettings(
    ...
    if (filter != null) {
      _filterText = filter;
      if (filter.isNotEmpty) _isFilterSpecific = true;
      needRefilter = true;
    }
    if (isRegex != null) {
    ...
    if (needRescan) {
      ...
      _filterTimer?.cancel();
      _filterTimer = Timer(Duration(milliseconds: filter != null ? 300 : 20),
          () => _applyFilters());
    }
    _saveState();
  }
    ...
    final input = {
      'files': _allFiles
          .map((f) => {
                ...
                'isHidden': f.originalName.startsWith('.')
              })
          .toList(),
      'filterText': _filterText,
      'isFilterRegex': _isFilterRegex,
      'hideSystemFiles': _hideSystemFiles,
      'showFolders': _showFolders
    };
    final visibility = await compute(_computeFilter, input);
    if (currentVersion != _filterVersion) return;
    final List<FileModel> filtered = [];
    for (int i = 0; i < _allFiles.length; i++) {
      if (visibility[i]) filtered.add(_allFiles[i]);
    }
  ...
  static List<bool> _computeFilter(Map<String, dynamic> params) {
    final List<dynamic> files = params['files'];
    final String filterText = params['filterText'];
    final bool isFilterRegex = params['isFilterRegex'];
    final bool hideSystemFiles = params['hideSystemFiles'];
    final bool showFolders = params['showFolders'];
    RegExp? regex;
    if (filterText.isNotEmpty && isFilterRegex) {
      try {
        regex = RegExp(filterText, caseSensitive: false, unicode: true);
      } catch (_) {}
    }
    return files.map((f) {
      final String originalName = f['originalName'];
      final bool isDirectory = f['isDirectory'];
      final bool isHidden = f['isHidden'];
      if (hideSystemFiles && isHidden) return false;
      if (!showFolders && isDirectory) return false;
      if (filterText.isNotEmpty) {
        if (isFilterRegex) {
          if (regex == null) return true;
          return regex.hasMatch(originalName);
        } else {
  ...
  bool _shouldShowFile(FileModel file) {
    ...
    if (_filterText.isNotEmpty) {
      if (_isFilterRegex) {
        try {
          return RegExp(_filterText, caseSensitive: false, unicode: true)
  ...
  void updateRenameSettings(
    ...
    if (mode != null) {
      if (isSubMode(mode)) {
        ...
        _lastExtraMode = mode;
      else if (isEtcMode(mode)) _lastEtcMode = mode;
    ...
    if (immediate) {
      ...
      _previewTimer =
          Timer(const Duration(milliseconds: 200), () => _updatePreviews());
    }
    _saveState();
    notifyListeners();
  }
  ...
  Future<void> _updatePreviews() async {
    ...
    try {
      ...
      const buffer = 20;
      final start =
          (_visibleStartIndex - buffer).clamp(0, _currentFiles.length);
      ...
      String? curFind = (_renameMode == RenameMode.deleteFrontTo ||
              _renameMode == RenameMode.deleteBackTo)
          ? _deleteToText
          : _findText;
      String? curReplace = (_renameMode == RenameMode.extension)
          ? _extensionChangeText
          : (_renameMode == RenameMode.extensionAdd
              ? _extensionAddText
              : _replaceText);
      String? baseDir = _currentDirectory != null
      ...
      final results =
          await compute(RenameEngine.computeGeneratePreviews, input);
  ...
  void _updatePreviewsDebounced({bool immediate = false}) {
    _previewTimer?.cancel();
    if (immediate) {
      _updatePreviews();
    } else {
      _previewTimer =
          Timer(const Duration(milliseconds: 50), () => _updatePreviews());
    }
  }
  ...
  void selectRange(int start, int end,
    ...
    for (int i = 0; i < _currentFiles.length; i++) {
      ...
      if (baseStates != null && i < baseStates.length) {
        _currentFiles[i].setSelected(isInside ? !baseStates[i] : baseStates[i],
            notify: false);
      } else {
        if (isInside) {
          _currentFiles[i].setSelected(true, notify: false);
        } else if (exclusive)
          _currentFiles[i].setSelected(false, notify: false);
      }
    }
    for (var f in _currentFiles) {
      f.notifyIfChanged();
  ...
  List<String> get backHistory => _navIndex <= 0
      ? []
      : _navHistory.sublist(0, _navIndex).reversed.toSet().toList();
  List<String> get forwardHistory => _navIndex >= _navHistory.length - 1
      ? []
      : _navHistory.sublist(_navIndex + 1).toSet().toList();
  Future<void> jumpBack(int steps) async {
    final ni = _navIndex - steps;
    if (ni >= 0) {
      _navIndex = ni;
      await _navigateInternal(Directory(_navHistory[_navIndex]));
    }
  ...
  Future<void> cancelScan() async {
    ...
    final receivePort = _currentReceivePort;
  ...
  Future<int> executeRename() async {
    final t = _currentFiles.where((f) => f.isSelected).toList();
    if (t.isEmpty) return 0;
    _isLoading = true;
    notifyListeners();
    List<FileModel> renamed = [];
    List<UndoAction> transaction = [];
    for (var file in t) {
  ...
  Future<void> renameOneFile(FileModel file, String newName) async {
    ...
    try {
      final oldP = file.entity.path;
      final newP = p.join(file.parentPath, newName);
      await file.entity.rename(newP);
      _undoManager
    ...
    for (var f in t) {
      try {
        if (!kIsWeb && Platform.isWindows) {
          ...
          final ps = f.entity.path.replaceAll("'", "''");
          final script =
              "Add-Type -AssemblyName Microsoft.VisualBasic; [Microsoft.VisualBasic.FileIO.FileSystem]::Delete${isDir ? 'Directory' : 'File'}('$ps', 'OnlyErrorDialogs', 'SendToRecycleBin')";
  ...
  static Future<bool> _directoryExistsWithTimeout(Directory directory,
  ...
  static Future<List<Directory>> getQuickAccessDirectories() async {
    ...
    if (Platform.isAndroid) {
      ...
      for (var f in folders) {
        ...
        if (await _directoryExistsWithTimeout(d)) qa.add(d);
      }
      return qa;
    }
    ...
    if (home != null) {
      final hd = Directory(home);
      if (await _directoryExistsWithTimeout(hd)) {
        qa.add(hd);
        for (var f in [
          'Desktop',
          'Downloads',
          ...
          final d = Directory(p.join(home, f));
          if (await _directoryExistsWithTimeout(d)) qa.add(d);
        }
        final od = Directory(p.join(home, 'OneDrive'));
        if (await _directoryExistsWithTimeout(od)) qa.add(od);
      }
    }
    return qa;
  }
  ...
  static Future<List<Directory>> getLogicalDrives() async {
    ...
    if (Platform.isWindows) {
      final fastDrives = await _getWindowsLogicalDrivesFast();
      if (fastDrives.isNotEmpty) return fastDrives;
      ...
      final exists = await Future.wait(
          candidates.map((d) => _directoryExistsWithTimeout(d)));
      for (var i = 0; i < candidates.length; i++) {
        if (exists[i]) ds.add(candidates[i]);
      }
    } else if (Platform.isAndroid) {
      final d = Directory('/storage/emulated/0');
      if (await _directoryExistsWithTimeout(d)) ds.add(d);
    } else
      ds.add(Directory('/'));
    return ds;
  }

  static Future<List<Directory>> _getWindowsLogicalDrivesFast() async {
    try {
      ...
      if (kDebugMode) debugPrint('Fast drive probe failed: $e');
      return [];
    }
  }
  ...
  bool get canExecute {
    ...
    // 日時変更または属性変更モードの場合は、ファイル名に変更がなくても実行可能とする
  ...
  bool get hasInvalidFilenamesSelected => _currentFiles
  ...
  void reorderFiles(int oldIndex, int newIndex) {
    ...
    if (draggedFile.isSelected) {
      ...
      int finalStartIdx =
          (insertBase - leaderOffset).clamp(0, _currentFiles.length);
  ...
  Future<void> goUp() async {
    if (_currentDirectory != null) {
      final p = _currentDirectory!.parent;
      if (p.path != _currentDirectory!.path) await setDirectory(p);
    }
  }
  ...
  List<UndoAction> getLastUndoTransaction() =>
      _undoManager.peekLastTransaction();
}
